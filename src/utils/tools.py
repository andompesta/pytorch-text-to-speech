...
def to_device(
    ...
    if len(data) == 6:
        ...
        speakers = torch.tensor(speakers).long().to(device)
...
def synth_samples(
    ...
    path: str,
    do_plot_mel: bool = False
):

    # ids = batch[0]
    # for i in range(len(predictions[0])):
    #     id_ = ids[i]
    #     src_len = predictions[8][i]
    ...
    #         )
    #         plt.savefig(os.path.join(path, "{}.png".format(id_)))
    #         plt.close()
...
def pad(
    ...
    mel_max_length: Optional[int] = None
):
    if mel_max_length:
