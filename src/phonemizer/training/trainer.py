...
class Trainer:
        ...
        for epoch in range(start_epoch + 1, config["training"]["epochs"] + 1):
            ...
            for i, batch in pbar:
                ...
                pbar.set_description(
                    desc=f"Epoch: {epoch} | Step {step} " f"| Loss: {avg_loss:#.4}",
                    refresh=True,
                )
                pred = model(batch)
    ...
    def _generate_samples(
        self, model: Model, preprocessor: Preprocessor, val_batches: List[dict]
    ...
    def _write_summaries(
        ...
        self.writer.add_scalar(
            f"Phoneme_Error_Rate/mean", eval_result["mean_per"], global_step=step
        )
        self.writer.add_scalar(
            f"Word_Error_Rate/mean", eval_result["mean_wer"], global_step=step
        )

        for lang in lang_samples.keys():
            result = eval_result[lang]
            self.writer.add_scalar(
                f"Phoneme_Error_Rate/{lang}", result["per"], global_step=step
            )
            self.writer.add_scalar(
                f"Word_Error_Rate/{lang}", result["wer"], global_step=step
            )
        ...
        for lang, samples in lang_samples.items():
            ...
            self.writer.add_text(
                f"{lang}/text_prediction_target", log_text, global_step=step
            )
