...
def nvidia_tacotron2(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        ckpt_file = _download_checkpoint(checkpoint, force_reload)
        ckpt = torch.load(kwargs.get("path_", "nvidia_tacotron2pyt_fp32_20190306.pth"), map_location="cpu")
...
def nvidia_waveglow(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        ckpt_file = _download_checkpoint(checkpoint, force_reload)
        ckpt = torch.load(kwargs.get("path_", "nvidia_waveglowpyt_fp32_20190306.pth"), map_location="cpu")
        state_dict = ckpt['state_dict']
        if checkpoint_from_distributed(state_dict):
            state_dict = unwrap_distributed(state_dict)
        config = ckpt['config']
